...
# # Function to save the result to Excel with all original sheets
# def to_excel_with_sheets(original_sheets, result_df, frequency_combined_df):
#     output = BytesIO()
#     with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
#         for sheet_name, sheet_df in original_sheets.items():
#             sheet_df.to_excel(writer, index=False, sheet_name=sheet_name)
...
#         st.download_button(
#             label="Download Predictions and Frequency Counts as Excel",
#             data=to_excel_with_sheets(original_sheets, result_df, frequency_combined_df),
#             file_name='predictions_and_frequency_counts.xlsx',
#             mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
#         )
#     else:
